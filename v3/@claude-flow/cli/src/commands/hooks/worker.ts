/**
 * Worker hook command: background worker management (12 workers)
 */
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const result = await callMCPTool<{
        workers: Array<{ trigger: string; description: string; priority: string; estimatedDuration: string; capabilities: string[]; patterns: number }>;
        total: number;
        active: { instances: Array<{ id: string; trigger: string; status: string; progress: number; phase: string }>; count: number; byStatus: Record<string, number> };
        performanceTargets: Record<string, string | number>;
      }>('hooks_worker-list', { status: ctx.flags['status'] || 'all', includeActive: ctx.flags['active'] !== false });
      spinner.succeed('Workers loaded');
      output.writeln(); output.writeln(output.bold('Background Workers (12 Total)')); output.writeln();
      output.printTable({
        columns: [
        ...
        data: result.workers.map(w => ({
          ...
          priority: w.priority === 'critical' ? output.error(w.priority) : w.priority === 'high' ? output.warning(w.priority) : w.priority,
          estimatedDuration: w.estimatedDuration,
          description: w.description,
        })),
      });
      if (ctx.flags['active'] && result.active.count > 0) {
        output.writeln(); output.writeln(output.bold('Active Instances'));
        output.printTable({
          columns: [
          ...
          data: result.active.instances.map(w => ({
            id: w.id, trigger: w.trigger,
            status: w.status === 'running' ? output.highlight(w.status) : w.status === 'completed' ? output.success(w.status) : w.status === 'failed' ? output.error(w.status) : w.status,
            progress: `${w.progress}%`,
          })),
        });
      }
      ...
      if (error instanceof MCPClientError) { output.printError(`Worker error: ${error.message}`); }
...
const workerDispatchCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const result = await callMCPTool<{
        success: boolean; workerId: string; trigger: string; context: string; priority: string;
        config: { description: string; estimatedDuration: string; capabilities: string[] };
        status: string; error?: string;
      }>('hooks_worker-dispatch', { trigger, context, priority, background });
      if (!result.success) { spinner.fail(`Failed: ${result.error}`); return { success: false, exitCode: 1 }; }
      spinner.succeed(`Worker dispatched: ${result.workerId}`);
      output.writeln();
      output.printTable({
        columns: [{ key: 'field', header: 'Field', width: 18 }, { key: 'value', header: 'Value', width: 50 }],
        data: [
      ...
      if (background) { output.writeln(); output.writeln(output.dim(`Check status: claude-flow hooks worker status --id ${result.workerId}`)); }
      return { success: true, data: result };
    } catch (error) {
      spinner.fail('Worker dispatch failed');
      if (error instanceof MCPClientError) { output.printError(`Dispatch error: ${error.message}`); }
      return { success: false, exitCode: 1 };
    }
  }
};
...
const workerStatusCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    try {
      const result = await callMCPTool<{
        success: boolean;
        worker?: { id: string; trigger: string; context: string; status: string; progress: number; phase: string; duration: number };
        workers?: Array<{ id: string; trigger: string; status: string; progress: number; phase: string; duration: number }>;
        summary?: { total: number; running: number; completed: number; failed: number };
        error?: string;
      }>('hooks_worker-status', { workerId, includeCompleted });
      if (!result.success) { spinner.fail(`Failed: ${result.error}`); return { success: false, exitCode: 1 }; }
      spinner.succeed('Status retrieved');
      if (result.worker) {
        output.writeln(); output.writeln(output.bold(`Worker: ${result.worker.id}`));
        output.printTable({
          columns: [{ key: 'field', header: 'Field', width: 15 }, { key: 'value', header: 'Value', width: 40 }],
          data: [
        ...
        output.writeln(); output.writeln(output.bold('Active Workers'));
        output.printTable({
          columns: [
            { key: 'id', header: 'Worker ID', width: 35 },
            { key: 'trigger', header: 'Type', width: 12 },
          ...
          data: result.workers.map(w => ({ id: w.id, trigger: w.trigger, status: formatWorkerStatus(w.status), progress: `${w.progress}%`, duration: `${w.duration}ms` })),
        });
        if (result.summary) {
          output.writeln();
        ...
        output.writeln(); output.writeln(output.dim('No active workers'));
      }
      return { success: true, data: result };
    } catch (error) {
      spinner.fail('Status check failed');
      if (error instanceof MCPClientError) { output.printError(`Status error: ${error.message}`); }
      return { success: false, exitCode: 1 };
    }
  }
};
...
const workerDetectCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    ...
    if (!prompt) { output.printError('--prompt is required'); return { success: false, exitCode: 1 }; }
    const spinner = output.createSpinner({ text: 'Analyzing prompt...', spinner: 'dots' });
    spinner.start();
    try {
      const result = await callMCPTool<{
        prompt: string;
        detection: { detected: boolean; triggers: string[]; confidence: number; context: string };
        triggersFound: number;
        triggerDetails?: Array<{ trigger: string; description: string; priority: string }>;
        autoDispatched?: boolean; workerIds?: string[];
      }>('hooks_worker-detect', { prompt, autoDispatch, minConfidence });
      if (result.detection.detected) { spinner.succeed(`Detected ${result.triggersFound} worker trigger(s)`); }
      else { spinner.succeed('No worker triggers detected'); }
      output.writeln(); output.writeln(output.bold('Detection Results'));
      ...
      if (result.triggerDetails && result.triggerDetails.length > 0) {
        ...
        output.printTable({
          ...
          data: result.triggerDetails.map(t => ({ trigger: output.highlight(t.trigger), priority: t.priority, description: t.description })),
        });
      }
      if (result.autoDispatched && result.workerIds) {
        output.writeln(); output.writeln(output.success('Workers auto-dispatched:'));
        result.workerIds.forEach(id => { output.writeln(`  - ${id}`); });
      }
      return { success: true, data: result };
    } catch (error) {
      spinner.fail('Detection failed');
      if (error instanceof MCPClientError) { output.printError(`Detection error: ${error.message}`); }
      return { success: false, exitCode: 1 };
    }
  }
};
...
const workerCancelCommand: Command = {
  ...
  action: async (ctx: CommandContext): Promise<CommandResult> => {
    const workerId = ctx.flags['id'] as string;
    if (!workerId) { output.printError('--id is required'); return { success: false, exitCode: 1 }; }
    const spinner = output.createSpinner({ text: `Cancelling worker ${workerId}...`, spinner: 'dots' });
    spinner.start();
    try {
      const result = await callMCPTool<{ success: boolean; workerId: string; cancelled: boolean; error?: string }>('hooks_worker-cancel', { workerId });
      if (!result.success) { spinner.fail(`Failed: ${result.error}`); return { success: false, exitCode: 1 }; }
      spinner.succeed(`Worker ${workerId} cancelled`);
      return { success: true, data: result };
    } catch (error) {
      spinner.fail('Cancel failed');
      if (error instanceof MCPClientError) { output.printError(`Cancel error: ${error.message}`); }
      return { success: false, exitCode: 1 };
    }
  }
};
...
export const workerCommand: Command = {
  ...
  action: async (): Promise<CommandResult> => {
    output.writeln(); output.writeln(output.bold('Background Worker System (12 Workers)')); output.writeln();
    output.writeln('Manage and dispatch background workers for analysis and optimization tasks.');
    output.writeln(); output.writeln('Available Workers:');
    output.printList([
    ...
    output.writeln(); output.writeln('Subcommands:');
    output.printList([
    ...
    output.writeln(); output.writeln('Run "claude-flow hooks worker <subcommand> --help" for details');
    return { success: true };
  }
};
